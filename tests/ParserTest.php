...
TXT;
    ...
    public function testParseFileWithEscapedCharactersInValues()
    {
        ...
        $this->assertEquals('Albania', $rows[2][0]);
        $this->assertEquals('This is "the Republic" of Albania', $rows[2][1]);
        $this->assertEquals('Algeria', $rows[3][0]);
        $expectedAlgeria = <<<TXT
the "People's" Democratic Republic
of Algeria
TXT;
        $this->assertEquals($expectedAlgeria, $rows[3][1]);
        $this->assertEquals('America', $rows[4][0]);
        $expectedAmerica = <<<TXT
the \"People's\"
of America
TXT;
        $this->assertEquals($expectedAmerica, $rows[4][1]);
    }
